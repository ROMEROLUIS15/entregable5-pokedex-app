  ...
  return (
   <main className="text-center capitalize">
    ...
    <article className="max-w-[500px] mx-auto py-10 px-2 relative pt-14 border-2 mt-[200px]">
      <header className={`absolute w-full top-0 right-0 -translate-y-2/3 rounded-sm ${bgByType[firstType]}`}>
    ...
    <article className="max-w-[500px] mx-auto py-10 px-2 pt-14  border-2 mt-10 mb-10 bg-white rounded-sm">
      ...
      <ul className="grid grid-cols-4 gap-2 mt-7">
        {
          pokemon?.moves.slice(0,30).map((move) => (
            <li key={move.move.url} className="border-2 rounded-sm border-black bg-slate-300">{move.move.name}
